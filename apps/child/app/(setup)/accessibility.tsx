...
export default function AccessibilityScreen() {
  ...
  const checkPermission = useCallback(() => {
    try {
      setGranted(hasAccessibilityPermission());
    } catch {}
  }, []);
  ...
  function handleRequest() {
    try { requestAccessibility(); } catch {}
  }
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.content}>
        ...
        <Text style={s.title}>Специальные возможности</Text>
        <Text style={s.desc}>
          Помогает вовремя напоминать, что пора сделать домашнее задание или поспать.
        </Text>

        <View style={s.bubble}>
          <Text style={s.bubbleText}>
            Нажми "Разрешить" → найди Kakai → включи
          </Text>
        </View>
        ...
        <TouchableOpacity
          style={s.nextBtn}
          onPress={() => router.push('/(setup)/overlay')}
          activeOpacity={0.85}
        >
