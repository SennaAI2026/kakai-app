...
export default function UsageStatsScreen() {
  ...
  const checkPermission = useCallback(() => {
    try {
      setGranted(hasUsageStatsPermission());
    } catch {
      // native module not available (dev/web)
    }
  }, []);
  ...
  function handleRequest() {
    try {
      ...
      // fallback handled by re-check on return
    }
  }
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.content}>
        ...
        <Text style={s.title}>Статистика приложений</Text>
        <Text style={s.desc}>
          Мама и папа увидят, сколько времени ты проводишь в играх и видео — чтобы ты не переусердствовал.
        </Text>

        <View style={s.bubble}>
          <Text style={s.bubbleText}>
            Нажми "Разрешить" → найди Kakai → включи переключатель
          </Text>
        </View>
        ...
        <TouchableOpacity
          style={s.nextBtn}
          onPress={() => router.push('/(setup)/accessibility')}
          activeOpacity={0.85}
        >
...
const s = StyleSheet.create({
  ...
  checkBadge: {
    ...
    borderWidth: 3, borderColor: '#FFF8E6',
  },
