...
export default function OverlayScreen() {
  ...
  const checkPermission = useCallback(() => {
    try {
      setGranted(hasOverlayPermission());
    } catch {}
  }, []);
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.content}>
        ...
        <Text style={s.title}>Поверх других окон</Text>
        <Text style={s.desc}>
          Покажет уведомление, когда экранное время заканчивается.
        </Text>
        ...
        <TouchableOpacity
          style={s.nextBtn}
          onPress={() => router.push('/(setup)/device-admin')}
          activeOpacity={0.85}
        >
